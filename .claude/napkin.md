...
6. **[2026-07-02, reinforced 2026-07-05] No commit/push/PR ceremony — git is autonomous; NEVER wait for merge approval (supersedes the kickoff-era "commits only with go-ahead" rule AND the stale "one user OK per merge" groove in the #6 handoff)**
   Do instead: commit and push without asking; run the CLAUDE.md 8-step flow END TO END — including step 6 (spawn multiple code-review subagents), step 7 (fix their findings), and step 8 (merge the PR + close the issue) — with NO stop to ask "OK to merge?". Fernando got frustrated twice by merge-approval asks; the self-merge is expected, not declined. Confirm only genuinely destructive/irreversible ops (history rewrite, force-push, deleting remote branches).
